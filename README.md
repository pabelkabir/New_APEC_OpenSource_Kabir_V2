...
2.	New_APEC.sh (New_APEC.sh): This is the first script to run to start a calculation.
...
4.	Solvent_box.sh (template/ASEC/Solvent_box.sh): The Chromophore has not been used so far in the previous steps. Only if performing mutations at the very beginning, in which case the chromophore is temporarily added to the original .pdb file. But no parametrization of the chromophore is needed to do that.
...
The parametrization is already in rtp format: template/ASEC/manchester_FMN_rtp, so, it is just to add it to amber99sb.ff/aminoacids.rtp.
...
8.	MD_2_QMMM.sh (template/ASEC/MD_2_QMMM.sh):
...
9.	Molcami_OptSCF.sh: (template/ASEC/Molcami_OptSCF.sh)
...
10.	ASEC.sh (template/ASEC/ASEC.sh)
...
Copy the script template/ASEC/Total_QMMM_Energy_MD.sh to Step_3.
